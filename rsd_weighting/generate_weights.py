...
def bin_pair_results(pair_results, nrp_bins=100, npar_bins=150, rp_max=20, par_max=30):
    ...
    For each of the 2D bins in r_p and r_par, this function calculates the IA signal and correlation function.
...
def get_rsd_ia_weights(cf_binned, ia_binned):
    ...
    slice_sum = np.sum(ia_binned**2 / (1 + cf_binned), axis=1)
    slice_weights = (ia_binned / (1 + cf_binned)) / slice_sum[:, np.newaxis]
