...
def plot_groupAlignment_results(result_paths, random_paths=None, groupCorr_paths=None, labels=['label'], colors=['b'], linestyles=['-'],
    ...
    for i in range(len(result_paths)):
        ...
        plt.errorbar(xvalues, xvalues*ang_plot, yerr=xvalues*ang_plot_e, label=labels[i], alpha=1, color=colors[i],
                    marker='o', markersize=4, capsize=3, elinewidth=1, linewidth=1.8, linestyle=linestyles[i], zorder=0);
    ...
    if np.all(pimax==pimax[0]):
        ...
        for j in range(len(results)):
            ...
            plt.text((results['R_bin_min'][j]+results['R_bin_max'][j])/2, -.3, str(round(results['pimax'][j], 2)), fontsize=fts-5, ha='center', alpha=.5)
    else:
    ...
    plt.legend(fontsize=fts-3, loc='lower left', bbox_to_anchor=(0.25, 0.15))
